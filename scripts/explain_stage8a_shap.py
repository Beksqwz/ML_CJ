...
def representative_sample(frame: pd.DataFrame, target: str, n: int) -> pd.DataFrame:
    ...
    chosen = np.concatenate(
        (
            rng.choice(positive, desired_pos, replace=False),
            rng.choice(negative, desired_neg, replace=False),
        )
    )
...
def factor_rows(
    features: list[str], values: pd.Series, row_shap: np.ndarray, positive: bool
    ...
    return [
        {
            "feature": features[i],
            "feature_value": clean(values.iloc[i]),
            "shap_log_odds": float(row_shap[i]),
        }
        for i in order[:5]
    ]


def _legacy_russian_text(
    probability: float,
    label: int,
    predicted: bool,
    positive: list[dict],
    ...
    minus = (
        ", ".join(x["feature"] for x in negative[:3]) or "нет отрицательных факторов"
    )
    return (
        f"Модель оценила вероятность риска в {probability:.2%}; фактически {truth}, прогноз {decision}. "
...
def russian_text(
    ...
    plus = (
        ", ".join(str(item["feature"]) for item in positive[:3]) or "\u043d\u0435\u0442"
    )
    minus = (
        ", ".join(str(item["feature"]) for item in negative[:3]) or "\u043d\u0435\u0442"
    )
    return (
...
def plots(
    output: Path, importance: pd.DataFrame, shap_values: np.ndarray, x: pd.DataFrame
) -> None:
    ...
    fig.tight_layout()
    fig.savefig(output / "shap_summary_bar_top30.png", dpi=180)
    plt.close(fig)
    ...
    ax.set_yticks(range(30), top_forward.iloc[::-1]["feature"])
    ax.set_xlabel("SHAP value (log-odds)")
    ax.set_title("SHAP beeswarm: топ-30 признаков")
    fig.tight_layout()
    fig.savefig(output / "shap_beeswarm_top30.png", dpi=180)
    plt.close(fig)
...
def run(horizon: str, stage7d: bool = False) -> dict[str, object]:
    ...
    if stage7d:
        config_path = (
            ROOT / "reports" / "stage7d" / horizon / "stage7d_feature_config.json"
        )
        split_root = ROOT / "data" / "processed" / "stage7d"
        model_path = (
            ROOT / "models" / "stage7d" / f"catboost_{horizon}_weather_experiment.cbm"
        )
    ...
    if model_features != features:
        raise ValueError(
            f"Feature order mismatch for {horizon}: model and Stage 7A config differ"
        )
    leakage = sorted(set(features) & set(config["excluded_from_model_features"]))
    if leakage:
        raise ValueError(
            f"Leakage/excluded fields present in model features: {leakage}"
        )
    ...
    if stage7d:
        validation = pd.read_parquet(
            split_root / f"training_dataset_{horizon}_validation.parquet"
        )
        validation_x = prepare(validation, features, categorical)
        validation_probability = model.predict_proba(
            Pool(validation_x, cat_features=categorical, feature_names=features)
        )[:, 1]
        threshold = best_f1_threshold(
            validation[target].to_numpy(dtype=np.int8), validation_probability
        )
        threshold_rule = "Maximum F1 recalculated only on Stage 7D validation predictions; model was not retrained."
    else:
        threshold_file = sorted(
            (ROOT / "reports" / "stage7b" / horizon).glob("*/catboost_*_threshold.json")
        )[-1]
        threshold = float(
            json.loads(threshold_file.read_text(encoding="utf-8"))["threshold"]
        )
    ...
    shap_frame.insert(
        1, "datetime_hour", sample["datetime_hour"].astype(str).to_numpy()
    )
    ...
    importance = importance.sort_values(
        "mean_abs_shap_log_odds", ascending=False, kind="stable"
    ).reset_index(drop=True)
    importance.insert(0, "rank", np.arange(1, len(importance) + 1))
    importance.to_csv(
        output / "global_feature_importance.csv", index=False, encoding="utf-8-sig"
    )
    importance.head(30).to_csv(
        output / "global_top30_feature_importance.csv",
        index=False,
        encoding="utf-8-sig",
    )
    plots(output, importance, shap_values, x)
    group = (
        importance.groupby("feature_group", sort=False)["mean_abs_shap_log_odds"]
        .sum()
        .sort_values(ascending=False)
        .reset_index(name="sum_mean_abs_shap_log_odds")
    )
    group["share_percent"] = (
        group["sum_mean_abs_shap_log_odds"]
        / group["sum_mean_abs_shap_log_odds"].sum()
        * 100
    )
    group.to_csv(
        output / "feature_group_contributions.csv", index=False, encoding="utf-8-sig"
    )
    ...
    for name, indices in categories.items():
        ...
        for i in pick:
            ...
            rows.append(
                {
                    "category": name,
                    "sample_row_index": int(sample.index[i]),
                    "datetime_hour": clean(sample.iloc[i]["datetime_hour"]),
                    "road_segment_id": clean(sample.iloc[i].get("road_segment_id")),
                    "probability": float(probability[i]),
                    "true_label": int(y[i]),
                    "predicted_high_risk": bool(predicted[i]),
                    "threshold": threshold,
                    "top_positive_shap_factors": pos,
                    "top_negative_shap_factors": neg,
                    "feature_values": {f: clean(x.iloc[i][f]) for f in features},
                    "explanation_ru": russian_text(
                        float(probability[i]), int(y[i]), bool(predicted[i]), pos, neg
                    ),
                }
            )
        local[name] = rows
    (output / "local_explanations.json").write_text(
        json.dumps(local, ensure_ascii=False, indent=2, default=clean), encoding="utf-8"
    )
    local_flat = pd.DataFrame(
        [
            {
                "category": r["category"],
                "sample_row_index": r["sample_row_index"],
                "datetime_hour": r["datetime_hour"],
                "road_segment_id": r["road_segment_id"],
                "probability": r["probability"],
                "true_label": r["true_label"],
                "predicted_high_risk": r["predicted_high_risk"],
                "explanation_ru": r["explanation_ru"],
                "feature_values_json": json.dumps(
                    r["feature_values"], ensure_ascii=False
                ),
                "positive_factors_json": json.dumps(
                    r["top_positive_shap_factors"], ensure_ascii=False
                ),
                "negative_factors_json": json.dumps(
                    r["top_negative_shap_factors"], ensure_ascii=False
                ),
            }
    ...
    single_share = float(
        importance.iloc[0]["mean_abs_shap_log_odds"]
        / importance["mean_abs_shap_log_odds"].sum()
    )
    group_share = float(group.iloc[0]["share_percent"] / 100)
    anomalies = []
    if single_share >= 0.50:
        anomalies.append(
            f"Один признак {importance.iloc[0]['feature']} даёт {single_share:.1%} суммарного mean|SHAP| (порог проверки 50%)."
        )
    if group_share >= 0.70:
        anomalies.append(
            f"Одна группа {group.iloc[0]['feature_group']} даёт {group_share:.1%} суммарного mean|SHAP| (порог проверки 70%)."
        )
    if not anomalies:
        anomalies.append(
            "Подозрительного доминирования одного признака (>=50%) или группы (>=70%) не обнаружено на выборке."
        )
    report = {
        "generated_at_utc": datetime.now(UTC).isoformat(),
        "horizon": horizon,
        "final_model_stage": model_stage,
        "model_path": str(model_path.resolve()),
        "feature_config": str(config_path.resolve()),
        "test_split": str(
            (split_root / f"training_dataset_{horizon}_test.parquet").resolve()
        ),
        "test_rows": len(test),
        "sample_rows": len(sample),
        "sample_positive_rate": float(y.mean()),
        "sampling": "Детерминированная стратифицированная случайная выборка test с seed=20260711.",
        "feature_order_matches_model": True,
        "leakage_fields_in_features": leakage,
        "threshold": threshold,
        "threshold_rule": threshold_rule,
        "shap_scale": "log-odds (RawFormulaVal)",
        "shap_additivity": {
            "max_abs_residual": float(np.abs(residual).max()),
            "mean_abs_residual": float(np.abs(residual).mean()),
            "passed_tolerance_1e-8": bool(np.abs(residual).max() < 1e-8),
        },
        "top_10_features": importance.head(10).to_dict(orient="records"),
        "feature_group_contributions": group.to_dict(orient="records"),
        "anomaly_check": anomalies,
        "local_example_counts": {k: len(v) for k, v in local.items()},
        "caution_ru": "SHAP отражает вклад признаков в выход конкретной модели на выбранных данных; это не причинная интерпретация.",
    }
    (output / "stage8a_report.json").write_text(
        json.dumps(report, ensure_ascii=False, indent=2, default=clean),
        encoding="utf-8",
    )
    return report
...
def main() -> None:
    ...
    parser.add_argument(
        "--stage7d-1h",
        action="store_true",
        help="Refresh only Stage 8A 1h using the accepted Stage 7D model.",
    )
    parser.add_argument(
        "--repair-local-text",
        action="store_true",
        help="Repair only UTF-8 Russian text in existing local_explanations.json files.",
    )
        ...
        for horizon in ("1h", "24h"):
            ...
            for rows in payload.values():
                for row in rows:
                    ...
                    row["explanation_ru"] = (
                        explanation
                        + " \u0424\u0430\u043a\u0442\u043e\u0440\u044b \u043e\u0431\u044a\u044f\u0441\u043d\u044f\u044e\u0442 \u0432\u043a\u043b\u0430\u0434 \u0432 \u043f\u0440\u043e\u0433\u043d\u043e\u0437, \u0430 \u043d\u0435 \u0434\u043e\u043a\u0430\u0437\u044b\u0432\u0430\u044e\u0442 \u043f\u0440\u0438\u0447\u0438\u043d\u043d\u043e\u0441\u0442\u044c."
                    )
            path.write_text(
    ...
    if args.stage7d_1h:
        reports = {
            "1h": run("1h", stage7d=True),
            "24h": json.loads(
                (OUT / "24h" / "stage8a_report.json").read_text(encoding="utf-8")
            ),
        }
    else:
        reports = {h: run(h) for h in ("1h", "24h")}
    group_1 = {
        x["feature_group"]: x["share_percent"]
        for x in reports["1h"]["feature_group_contributions"]
    }
    group_24 = {
        x["feature_group"]: x["share_percent"]
        for x in reports["24h"]["feature_group_contributions"]
    }
    ...
    comparison = {
        ...
        "final_models": {
            "1h": {
                "stage": reports["1h"].get("final_model_stage", "Stage 7B"),
                "model_path": reports["1h"].get("model_path"),
            },
            "24h": {
                "stage": reports["24h"].get("final_model_stage", "Stage 7B"),
                "model_path": reports["24h"].get("model_path"),
            },
        },
        "comparison_ru": comparison_text,
        "1h_top_10": top1,
        "24h_top_10": top24,
        "1h_groups": reports["1h"]["feature_group_contributions"],
        "24h_groups": reports["24h"]["feature_group_contributions"],
    }
    (OUT / "stage8a_model_comparison.json").write_text(
