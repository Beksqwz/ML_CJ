...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train one CatBoost Stage 7B model with temporal Stage 7A splits."
    )
...
def load_baseline_report(label: str) -> tuple[dict[str, object], Path]:
    candidates = sorted(
        (PROJECT_ROOT / "reports" / "stage7a" / label).glob(
            "*/training_dataset_*_stage7a_report.json"
        )
    )
    if not candidates:
...
def roc_auc(y: np.ndarray, score: np.ndarray) -> float | None:
    ...
    return float(
        (ranks[y == 1].sum() - positive_count * (positive_count + 1) / 2)
...
def calibration_summary(y: np.ndarray, score: np.ndarray) -> dict[str, object]:
    ...
    return {
        ...
        "expected_calibration_error_10_bins": weighted_error / len(y),
        "bins": rows,
    }
...
def comparison(
    ...
    for key in keys:
        ...
        result[key] = {
            "absolute": absolute,
            "relative_percent": absolute / float(baseline_value) * 100
            if baseline_value
            else None,
        }
    return result
...
def train_with_gpu_fallback(
    ...
    try:
        ...
        return model, {
            ...
            "effective_params": cpu_params,
        }


def save_predictions(
    label: str, split: str, frame: pd.DataFrame, target: str, probability: np.ndarray
) -> str:
...
def main() -> int:
    ...
    splits = {
        name: pd.read_parquet(SPLIT_ROOT / f"training_dataset_{label}_{name}.parquet")
    ...
    model, training = train_with_gpu_fallback(
        params, train_x, train_y, valid_x, valid_y, cat_indexes
    )
    prediction = {
        "train": model.predict_proba(train_x)[:, 1],
        "validation": model.predict_proba(valid_x)[:, 1],
        "test": model.predict_proba(test_x)[:, 1],
    }
    threshold, validation_f1 = best_f1_threshold(valid_y, prediction["validation"])
    model_metrics = {
        name: metric_summary(labels, prediction[name], threshold)
        for name, labels in (
            ("train", train_y),
            ("validation", valid_y),
            ("test", test_y),
    ...
    calibration = {
        name: calibration_summary(labels, prediction[name])
        for name, labels in (("validation", valid_y), ("test", test_y))
    }
    predictions = {
        name: save_predictions(label, name, splits[name], target, prediction[name])
        ...
        json.dumps(
            {
                ...
                "excluded_features": config["excluded_from_model_features"],
            },
            ensure_ascii=False,
            indent=2,
        ),
    ...
    params_path.write_text(
        json.dumps(training, ensure_ascii=False, indent=2, default=str),
        encoding="utf-8",
    )
    threshold_path.write_text(
        json.dumps(
            {
                "rule": "Maximum F1 selected exclusively on validation probabilities.",
                "threshold": threshold,
    ...
    report = {
        ...
        "baseline_comparison": {
            name: comparison(model_metrics[name], baseline_metrics[name])
            for name in ("validation", "test")
        },
        "overfitting_check": {
            "train_validation_pr_auc_gap": float(
                model_metrics["train"]["pr_auc"] - model_metrics["validation"]["pr_auc"]
            ),
            "validation_test_pr_auc_gap": float(
                model_metrics["validation"]["pr_auc"] - model_metrics["test"]["pr_auc"]
            ),
            "train_validation_roc_auc_gap": float(
                model_metrics["train"]["roc_auc"]
                - model_metrics["validation"]["roc_auc"]
            ),
