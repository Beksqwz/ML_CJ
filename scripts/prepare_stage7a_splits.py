...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Create chronological Stage 7A data splits and baseline reports."
    )
    ...
    parser.add_argument(
        ...
        type=int,
        choices=(1, 24),
        help="Forecast horizon of the input dataset.",
    )
    parser.add_argument(
        "--label",
        choices=("1h", "24h"),
        help="Shortcut for a standard Stage 6 dataset.",
    )
    parser.add_argument(
        "--rare-min-train-rows", type=int, default=RARE_CATEGORY_MIN_TRAIN_ROWS
    )
...
def resolve_args(args: argparse.Namespace) -> tuple[Path, int, str]:
    if args.label:
        return (
            DEFAULT_DATASETS[args.label],
            int(args.label.removesuffix("h")),
            args.label,
        )
...
def chronological_splits(
    data: pd.DataFrame, horizon_hours: int
) -> tuple[dict[str, pd.DataFrame], dict[str, object]]:
    timestamps = (
        pd.Series(pd.to_datetime(data["datetime_hour"], errors="raise").unique())
        .sort_values()
    ...
    validation = data.loc[
        (timestamp >= first_boundary) & (timestamp < second_boundary - horizon)
    ].copy()
...
def feature_configuration(data: pd.DataFrame, target: str) -> dict[str, object]:
    ...
    return {
        ...
        "metadata_columns": [
            column
            for column in ("datetime_hour", "road_segment_id")
...
def apply_train_only_transforms(
    splits: dict[str, pd.DataFrame], config: dict[str, object], rare_min: int
) -> tuple[dict[str, pd.DataFrame], dict[str, object]]:
    train = splits["train"]
    ...
    for name, frame in splits.items():
        ...
        result["road_highway"] = road_highway.where(
            road_highway.isin(known), "OTHER"
        ).astype("string")
        result.loc[result["road_lanes_missing"].astype(bool), "road_lanes_num"] = np.nan
        result.loc[
            result["road_maxspeed_missing"].astype(bool), "road_maxspeed_kmh"
        ] = np.nan
    ...
    transform = {
        "road_highway_grouping": {
            ...
            "train_category_counts_before_grouping": {
                str(key): int(value) for key, value in frequency.items()
            },
        },
...
def pr_auc(y: np.ndarray, score: np.ndarray) -> float | None:
    ...
    grouped = (
        ...
        .groupby("score", sort=False)["target"]
        .agg(["count", "sum"])
...
def best_train_f1_threshold(y: np.ndarray, score: np.ndarray) -> float:
    ...
    false_positive = (grouped["count"].cumsum() - grouped["sum"].cumsum()).to_numpy(
        dtype=float
    )
...
def historical_baseline(
    splits: dict[str, pd.DataFrame], target: str
) -> dict[str, object]:
    train = splits["train"]
    global_rate = float(train[target].mean())
    grouped = train.groupby("road_segment_id")[target].agg(["sum", "count"])
    segment_rate = (
        (grouped["sum"] + SMOOTHING_ALPHA * global_rate)
        / (grouped["count"] + SMOOTHING_ALPHA)
    ).to_dict()
    train_score = (
        train["road_segment_id"]
        .map(segment_rate)
        .fillna(global_rate)
        .to_numpy(dtype=float)
    )
    threshold = best_train_f1_threshold(
        train[target].to_numpy(dtype=np.int8), train_score
    )
    ...
    for name in ("validation", "test"):
        frame = splits[name]
        score = (
            frame["road_segment_id"]
            .map(segment_rate)
            .fillna(global_rate)
            .to_numpy(dtype=float)
        )
        payload["metrics"][name] = metrics(
            frame[target].to_numpy(dtype=np.int8), score, threshold
        )
        payload["metrics"][name]["rows"] = int(len(frame))
        payload["metrics"][name]["positive_rows"] = int(frame[target].sum())
        payload["metrics"][name]["unseen_segments_fallback_to_global_rate"] = int(
            (~frame["road_segment_id"].isin(segment_rate)).sum()
        )
    return payload


def save_splits(
    splits: dict[str, pd.DataFrame], target: str, label: str, config: dict[str, object]
) -> dict[str, str]:
...
def main() -> int:
    ...
    transformed, transform = apply_train_only_transforms(
        splits, config, args.rare_min_train_rows
    )
    ...
    report = {
        ...
        "splits": {
            name: {
                ...
                "start": str(frame["datetime_hour"].min()),
                "end": str(frame["datetime_hour"].max()),
            }
