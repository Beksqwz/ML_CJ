...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Independently validate the 24-hour training dataset."
    )
...
def source_counts(ready: pd.DataFrame) -> pd.DataFrame:
    ...
    events["event_hour"] = pd.to_datetime(
        events["accident_datetime"], errors="raise"
    ).dt.floor("h")
    return (
        events.groupby(["road_segment_id", "event_hour"], as_index=False)
        .size()
        .rename(columns={"size": "event_count"})
    )


def expected_positive_keys(
    counts: pd.DataFrame, start: pd.Timestamp, end: pd.Timestamp
...
def standard_history_mismatches(
    data: pd.DataFrame, counts: pd.DataFrame
) -> dict[str, int]:
    columns = [
    ...
    for segment, indices in data.groupby("road_segment_id").groups.items():
        ...
        hours_since[has_history] = (
            query_ns[has_history] - event_ns[positions[has_history] - 1]
        ) / 3_600_000_000_000
        ...
        for column, values in expected.items():
            ...
            bad[column] += int(
                (
                    ~np.isclose(
                        actual.astype(float), values.astype(float), equal_nan=True
                    )
                ).sum()
            )
    city = (
    ...
    for label, hours in (
        ...
        expected_city[label] = total - prior_city(
            query_ns - np.int64(hours * 3_600_000_000_000)
        )
...
def seasonal_sample_mismatches(
    data: pd.DataFrame, counts: pd.DataFrame, calendar: pd.DataFrame
) -> dict[str, int]:
    """Independently recompute seasonal features for deterministic 100k-row audit sample."""
    sample = data.iloc[
        np.linspace(0, len(data) - 1, num=min(100_000, len(data)), dtype=int)
    ].copy()
    calendar = calendar[["datetime_hour", "is_holiday"]].copy()
    calendar["is_new_year"] = (
    ...
    def contextual_prior(
        query_columns: list[str], event_columns: list[str], event_subset: pd.DataFrame
    ) -> np.ndarray:
        ...
        right = event_subset.groupby(
            ["road_segment_id", "event_hour", *event_columns], as_index=False
        )["event_count"].sum()
        right = right.sort_values(["event_hour", *groups])
        right["_cum"] = right.groupby(groups, dropna=False)["event_count"].cumsum()
        merged = pd.merge_asof(
            left.sort_values(["datetime_hour", *groups]),
            right[["event_hour", *groups, "_cum"]],
        ...
        return (
            pd.Series(
                merged["_cum"].fillna(0).to_numpy(dtype=np.int64),
                index=merged["_row"].to_numpy(),
            )
            .reindex(np.arange(len(sample)), fill_value=0)
            .to_numpy()
        )
    ...
    events["_all"] = 1
    expected = {
        "segment_accidents_same_month_prior": contextual_prior(
            ["_month"], ["event_month"], events
        ),
        "segment_accidents_same_hour_weekday_prior": contextual_prior(
            ["_hour", "_weekday"], ["event_hour_of_day", "event_weekday"], events
        ),
        "segment_accidents_same_month_hour_weekday_prior": contextual_prior(
            ["_month", "_hour", "_weekday"],
            ["event_month", "event_hour_of_day", "event_weekday"],
            events,
        ),
        "segment_accidents_holiday_prior": contextual_prior(
            ["_all"], ["_all"], events.loc[events.is_holiday.fillna(False)]
        ),
        "segment_accidents_new_year_period_prior": contextual_prior(
            ["_all"], ["_all"], events.loc[events.is_new_year.fillna(False)]
        ),
    }
    return {
        column: int((sample[column].to_numpy(dtype=np.int64) != values).sum())
        for column, values in expected.items()
    }
...
def main() -> int:
    ...
    start, end = (
        max(calendar.datetime_hour.min(), weather.datetime_hour.min()),
        min(calendar.datetime_hour.max(), weather.datetime_hour.max()),
    )
    ...
    positive_keys = set(
        zip(positive.road_segment_id.astype(str), positive.datetime_hour)
    )
    negative_keys = set(
        zip(negative.road_segment_id.astype(str), negative.datetime_hour)
    )
    ...
    required_prefixes = {
        ...
        "poi": [c for c in data if c.startswith("poi_")],
        "calendar": [c for c in data if c.startswith("calendar_")],
        "weather": [c for c in data if c.startswith("weather_")],
    }
    joins = {
        name: int(data[cols].isna().any(axis=1).sum()) if cols else len(data)
        for name, cols in required_prefixes.items()
    }
    summary = {
        ...
        "validations": {
            "positive_labels": {
                "passed": positive_keys == expected,
                "expected": len(expected),
                "actual": len(positive_keys),
                "mismatches": len(expected.symmetric_difference(positive_keys)),
            },
            "negative_windows_accident_free": {
                "passed": len(negative_keys & expected) == 0,
                "overlapping_positive_windows": len(negative_keys & expected),
            },
            "duplicate_keys": {
                "passed": not data.duplicated(
                    ["road_segment_id", "datetime_hour"]
                ).any(),
                "count": int(
                    data.duplicated(["road_segment_id", "datetime_hour"]).sum()
                ),
            },
            "horizon_definition": {
                "passed": True,
                "definition": "positive keys independently rebuilt from events in (datetime_hour, datetime_hour + 24h]",
            },
            "temporal_coverage": {
                "passed": bool(data.datetime_hour.between(start, end).all()),
                "start": str(data.datetime_hour.min()),
                "end": str(data.datetime_hour.max()),
                "external_feature_start": str(start),
                "external_feature_end": str(end),
            },
            "class_balance": {
                "passed": len(negative) == len(positive) * 5,
                "positive": int(len(positive)),
                "negative": int(len(negative)),
                "negative_to_positive": len(negative) / len(positive),
            },
            "missing_values": {
                "passed": not bool(data.isna().any().any()),
                "nonzero": {
                    c: int(data[c].isna().sum()) for c in data if data[c].isna().any()
                },
            },
            "joins": {
                "passed": all(v == 0 for v in joins.values())
                and set(data.road_segment_id.astype(str)).issubset(source_segments),
                "rows_with_missing_joined_values": joins,
            },
            "standard_historical_features": {
                "passed": all(v == 0 for v in standard_bad.values()),
                "mismatches": standard_bad,
            },
            "seasonal_historical_features": {
                "passed": all(v == 0 for v in seasonal_bad.values()),
                "audit_sample_rows": min(100_000, len(data)),
                "mismatches": seasonal_bad,
            },
            "leakage": {
                "passed": not bool(FORBIDDEN & set(data.columns)),
                "forbidden_columns_present": sorted(FORBIDDEN & set(data.columns)),
                "current_weather_only": True,
                "future_target_events_not_used_in_history": all(
                    v == 0 for v in standard_bad.values()
                ),
            },
        },
    }
    summary["final_status"] = (
        "READY"
        if all(item["passed"] for item in summary["validations"].values())
        else "FAILED"
    )
