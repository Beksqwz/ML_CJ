...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Clean historical Astana accident CSV or Parquet data.")
    parser.add_argument("--input", type=Path, help="Source CSV or Parquet file. Defaults to astana_accidents.parquet.")
...
def resolve_input(input_path: Path | None) -> Path:
    ...
    for name in ("astana_accidents.parquet", "astana_accidents.csv"):
        ...
        if path.is_file():
            LOGGER.info("Automatically selected input: %s", path.relative_to(PROJECT_ROOT))
            return path
    raise FileNotFoundError("Neither astana_accidents.parquet nor astana_accidents.csv exists in the project root.")
...
def remove_duplicates(dataframe: pd.DataFrame) -> tuple[pd.DataFrame, int, int]:
    ...
    objectid_present = deduplicated["objectid"].notna() & deduplicated["objectid"].astype("string").str.strip().ne("")
    duplicate_objectids = objectid_present & deduplicated.duplicated(subset=["objectid"], keep="first")
    objectid_removed = int(duplicate_objectids.sum())
    deduplicated = deduplicated.loc[~duplicate_objectids].copy()
    LOGGER.info("Removed duplicates: full=%d, objectid=%d", full_removed, objectid_removed)
...
def create_datetime_columns(dataframe: pd.DataFrame) -> pd.DataFrame:
    ...
    date_values = pd.to_datetime(pd.to_numeric(result["rta_date"], errors="coerce"), unit="ms", errors="coerce", utc=True)
...
def create_time_features(dataframe: pd.DataFrame) -> pd.DataFrame:
    ...
    result["season"] = result["month"].map({12: "winter", 1: "winter", 2: "winter", 3: "spring", 4: "spring", 5: "spring", 6: "summer", 7: "summer", 8: "summer", 9: "autumn", 10: "autumn", 11: "autumn"}).astype("string")
...
def create_geodataframe(dataframe: pd.DataFrame) -> gpd.GeoDataFrame:
    ...
    valid_mercator = x.between(-20_037_508.35, 20_037_508.35) & y.between(-20_048_966.1, 20_048_966.1)
    geometry_3857 = gpd.GeoSeries(
        [Point(x_value, y_value) if is_valid else None for x_value, y_value, is_valid in zip(x, y, valid_mercator, strict=True)],
        index=result.index,
        crs="EPSG:3857",
    )
    geodataframe = gpd.GeoDataFrame(result, geometry=geometry_3857, crs="EPSG:3857").to_crs("EPSG:4326")
    geodataframe["longitude"] = geodataframe.geometry.x
    geodataframe["latitude"] = geodataframe.geometry.y
    return geodataframe


def remove_missing_required(dataframe: gpd.GeoDataFrame) -> tuple[gpd.GeoDataFrame, int, int, int]:
    """Drop records lacking an identifier, usable coordinates, or complete accident timestamp."""
    objectid_missing = dataframe["objectid"].isna() | dataframe["objectid"].astype("string").str.strip().eq("")
    without_objectid = dataframe.loc[~objectid_missing].copy()
    missing_objectid_removed = int(objectid_missing.sum())

    coordinate_missing = without_objectid["x"].isna() | without_objectid["y"].isna() | without_objectid.geometry.isna()
    ...
    LOGGER.info("Removed incomplete records: objectid=%d, coordinates=%d, datetime=%d", missing_objectid_removed, missing_coordinates_removed, missing_datetime_removed)
    return cleaned, missing_objectid_removed, missing_coordinates_removed, missing_datetime_removed


def write_suspicious_coordinates(dataframe: gpd.GeoDataFrame, report_dir: Path) -> tuple[Path, int]:
    ...
    in_region = longitude.between(ASTANA_REGION_BOUNDS[0], ASTANA_REGION_BOUNDS[2]) & latitude.between(ASTANA_REGION_BOUNDS[1], ASTANA_REGION_BOUNDS[3])
    ...
    LOGGER.info("Suspicious Astana-area coordinates retained and reported: %d", len(suspicious))
...
def write_reports(report_dir: Path, metrics: CleaningMetrics) -> None:
    ...
    payload: dict[str, Any] = asdict(metrics) | {"generated_at_utc": datetime.now(UTC).isoformat()}
    (report_dir / "cleaning_summary.json").write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
    lines = [
...
def main() -> int:
    ...
    try:
        ...
        cleaned_geodataframe, missing_objectid, missing_coordinates, missing_datetime = remove_missing_required(geodataframe)
        report_dir = make_report_dir()
        suspicious_report, suspicious_count = write_suspicious_coordinates(cleaned_geodataframe, report_dir)
        parquet_path, csv_path = save_outputs(cleaned_geodataframe)
        metrics = CleaningMetrics(
            input_path=str(input_path), rows_before=rows_before, rows_after=len(cleaned_geodataframe),
            full_duplicates_removed=full_duplicates, objectid_duplicates_removed=objectid_duplicates,
            missing_objectid_removed=missing_objectid, missing_coordinates_removed=missing_coordinates,
            missing_datetime_removed=missing_datetime, suspicious_coordinates=suspicious_count,
            created_columns=["accident_date", "accident_datetime", "hour", "weekday", "day", "month", "year", "quarter", "season", "is_weekend", "latitude", "longitude", "geometry"],
            output_parquet=str(parquet_path), output_csv=str(csv_path), suspicious_coordinates_report=str(suspicious_report),
        )
        ...
        print("python scripts/validate_historical_accidents.py --input data/processed/astana_accidents_clean.parquet")
        return 0
