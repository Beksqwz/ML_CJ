...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Create hourly Kazakhstan calendar features."
    )
    ...
    parser.add_argument(
        "--output-csv",
        type=Path,
        default=EXTERNAL_ROOT / "calendar_features_hourly.csv",
    )
...
def create_calendar(start_date: pd.Timestamp, end_date: pd.Timestamp) -> pd.DataFrame:
    hours = pd.date_range(
        start=start_date, end=end_date + pd.Timedelta(hours=23), freq="h"
    )
    years = range(int(start_date.year), int(end_date.year) + 1)
    kz_holidays = holidays.country_holidays(
        "KZ", years=years, observed=True, language="en_US"
    )
    holiday_dates = {
        pd.Timestamp(day).date(): name for day, name in kz_holidays.items()
    }
    ...
    data["is_holiday"] = (
        data["datetime_hour"]
        .dt.date.map(lambda day: day in holiday_dates)
        .astype("boolean")
    )
    data["holiday_name"] = (
        data["datetime_hour"]
        .dt.date.map(lambda day: holiday_dates.get(day, ""))
        .astype("string")
    )
    data["is_day_before_holiday"] = (
        (data["datetime_hour"].dt.date + pd.Timedelta(days=1))
        .map(lambda day: day in holiday_dates)
        .astype("boolean")
    )
    data["is_day_after_holiday"] = (
        (data["datetime_hour"].dt.date - pd.Timedelta(days=1))
        .map(lambda day: day in holiday_dates)
        .astype("boolean")
    )
    ...
    data["is_school_year"] = (
        data["month"].isin([9, 10, 11, 12, 1, 2, 3, 4, 5]).astype("boolean")
    )
    data["is_school_summer_break"] = data["month"].isin([6, 7, 8]).astype("boolean")
    data["is_school_winter_break"] = (
        ((data["month"] == 12) & data["day"].ge(29))
        | ((data["month"] == 1) & data["day"].le(8))
    ).astype("boolean")
    data["is_school_spring_break"] = (
        (data["month"] == 3) & data["day"].between(21, 31)
    ).astype("boolean")
    data["is_school_autumn_break"] = (
        (data["month"] == 10) & data["day"].ge(28)
    ).astype("boolean")
...
def write_report(report_dir: Path, summary: dict[str, Any]) -> None:
    (report_dir / "calendar_summary.json").write_text(
...
def main() -> int:
    ...
    try:
        ...
        summary = {
            ...
            "unique_holidays": sorted(
                name
                for name in calendar["holiday_name"].dropna().unique().tolist()
                if name
            ),
