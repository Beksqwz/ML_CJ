...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Attach each cleaned accident to the nearest Astana road segment.")
    parser.add_argument(
        "--accidents",
        type=Path,
    ...
    parser.add_argument("--output-csv", type=Path, default=PROCESSED_ROOT / "accidents_with_roads.csv")
    parser.add_argument("--output-parquet", type=Path, default=PROCESSED_ROOT / "accidents_with_roads.parquet")
...
def project_lonlat(lon: float, lat: float) -> tuple[float, float]:
    ...
    x = EARTH_RADIUS_M * math.radians(lon - ASTANA_LON0) * math.cos(math.radians(ASTANA_LAT0))
...
def bbox_for(points: Iterable[tuple[float, float]]) -> tuple[float, float, float, float]:
    xs, ys = zip(*points)
    return min(xs), min(ys), max(xs), max(ys)


def road_id(row: dict[str, str], fallback_index: int) -> str:
    u, v, key = row.get("u", "").strip(), row.get("v", "").strip(), row.get("key", "").strip()
    if u and v and key:
...
def load_roads(path: Path) -> list[RoadSegment]:
    ...
    with path.open("r", encoding="utf-8-sig", newline="") as file:
        ...
        if not reader.fieldnames or "geometry" not in reader.fieldnames:
            raise ValueError("Road CSV must contain a geometry column with LINESTRING WKT.")
        for index, row in enumerate(reader):
            points_m = parse_linestring_wkt(row.get("geometry", ""))
            if points_m is None:
                continue
            attributes = {column: row.get(column, "") for column in ROAD_ATTRIBUTE_COLUMNS}
            roads.append(
                RoadSegment(
...
def cell_range(min_value: float, max_value: float) -> range:
    return range(math.floor(min_value / GRID_CELL_M), math.floor(max_value / GRID_CELL_M) + 1)
...
def point_polyline_distance(px: float, py: float, points: tuple[tuple[float, float], ...]) -> float:
    best = math.inf
    for first, second in zip(points, points[1:]):
        distance = point_segment_distance(px, py, first[0], first[1], second[0], second[1])
        if distance < best:
            best = distance
    return best


def candidate_indices(px: float, py: float, grid: dict[tuple[int, int], list[int]], max_ring: int = 10) -> set[int]:
...
def try_write_parquet(csv_path: Path, parquet_path: Path) -> Path | None:
    try:
        ...
        LOGGER.warning("Skipping Parquet output because pandas/pyarrow are unavailable: %s", exc)
        return None
...
def match_accidents(args: argparse.Namespace) -> MatchingSummary:
    ...
    with accidents_path.open("r", encoding="utf-8-sig", newline="") as input_file, output_csv.open(
        ...
        if missing:
            raise ValueError(f"Clean accidents CSV is missing required columns: {', '.join(missing)}")
        ...
        writer = csv.DictWriter(output_file, fieldnames=[*reader.fieldnames, *road_fields])
        writer.writeheader()
    ...
    with far_matches_path.open("w", encoding="utf-8-sig", newline="") as far_file:
        fieldnames = [*reader.fieldnames, "road_segment_id", "distance_to_road_m", *[f"road_{column}" for column in ROAD_ATTRIBUTE_COLUMNS]]
    ...
    (report_dir / "road_matching_summary.json").write_text(
        json.dumps(asdict(summary) | {"generated_at_utc": datetime.now(timezone.utc).isoformat()}, ensure_ascii=False, indent=2),
        encoding="utf-8",
    )
...
def main() -> int:
    ...
    try:
        ...
        print(f"Distance p50/p90/p95/max m: {summary.distance_p50_m}/{summary.distance_p90_m}/{summary.distance_p95_m}/{summary.distance_max_m}")
