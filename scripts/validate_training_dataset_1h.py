...
FORBIDDEN_FEATURES = {
    ...
    "fd1r17_descrip",
    "distance_to_road_m",
    "target_event_count_1h",
}


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate stage-6 road-segment / hourly training dataset."
    )
...
def expected_positive_keys(ready: pd.DataFrame) -> set[tuple[str, pd.Timestamp]]:
    ...
    return set(
        zip(
            events["road_segment_id"].astype(str),
            events["event_hour"] - pd.Timedelta(hours=1),
        )
    )


def verify_strict_history(
    dataset: pd.DataFrame, ready: pd.DataFrame
) -> dict[str, object]:
    events = ready[["road_segment_id", "accident_datetime"]].copy()
    ...
    for segment, indices in dataset.groupby("road_segment_id").groups.items():
        group = counts.loc[counts["road_segment_id"] == segment].sort_values(
            "event_hour"
        )
        event_ns = (
            group["event_hour"].astype("datetime64[ns]").astype("int64").to_numpy()
        )
        cumulative = group["count"].cumsum().to_numpy(dtype=np.int64)
        query = (
            dataset.loc[list(indices), "datetime_hour"]
        ...
        actual = dataset.loc[list(indices), "segment_accidents_total_prior"].to_numpy(
            dtype=np.int64
        )
        mismatches += int((expected != actual).sum())
    city = (
        counts.groupby("event_hour", as_index=False)["count"]
        .sum()
    ...
    city_mismatches = int(
        (
            city_expected
            != dataset["city_accidents_total_prior"].to_numpy(dtype=np.int64)
        ).sum()
    )
    return {
        "segment_total_prior_mismatches": mismatches,
        "city_total_prior_mismatches": city_mismatches,
        "passed": mismatches == 0 and city_mismatches == 0,
    }
...
def main() -> int:
    ...
    data["target_datetime_hour"] = pd.to_datetime(
        data["target_datetime_hour"], errors="raise"
    )
    expected = expected_positive_keys(ready)
    actual_positive = set(
        zip(
            data.loc[data["target_1h"] == 1, "road_segment_id"].astype(str),
            data.loc[data["target_1h"] == 1, "datetime_hour"],
        )
    )
    negative_keys = set(
        zip(
            data.loc[data["target_1h"] == 0, "road_segment_id"].astype(str),
            data.loc[data["target_1h"] == 0, "datetime_hour"],
        )
    )
    ...
    join_columns = {
        ...
        "calendar": [
            column for column in data.columns if column.startswith("calendar_")
        ],
        "weather": [column for column in data.columns if column.startswith("weather_")],
    }
    join_missing = {
        name: int(data[columns].isna().any(axis=1).sum()) if columns else len(data)
        for name, columns in join_columns.items()
    }
    ...
    summary = {
        ...
        "target_balance": {
            "positive": positives,
            "negative": negatives,
            "negative_to_positive": negatives / positives if positives else None,
        },
        "temporal_coverage": {
        ...
        "duplicates": {
            "road_segment_datetime_hour": int(
        ...
        "label_checks": {
            ...
            "positive_key_mismatches": int(
                len(expected.symmetric_difference(actual_positive))
            ),
            "negative_positive_overlap": int(len(negative_keys.intersection(expected))),
        },
        "leakage_checks": {
            "forbidden_post_event_columns": forbidden_present,
            "target_timestamp_is_exactly_next_hour": bool(
                (
                    data["target_datetime_hour"]
                    == data["datetime_hour"] + pd.Timedelta(hours=1)
                ).all()
            ),
