...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build stage-6 24-hour training data without model training."
    )
...
def event_counts(ready: pd.DataFrame) -> pd.DataFrame:
    ...
    if getattr(timestamps.dt, "tz", None) is not None:
        raise ValueError(
            "accident_datetime must be timezone-naive local Asia/Almaty time."
        )
...
def make_positive_windows(
    counts: pd.DataFrame, feature_start: pd.Timestamp, feature_end: pd.Timestamp
    ...
    all_positive = pd.concat(pieces, ignore_index=True).drop_duplicates(
        ["road_segment_id", "datetime_hour"]
    )
    eligible = all_positive.loc[
        all_positive["datetime_hour"].between(feature_start, feature_end)
    ].copy()
...
def sample_negatives(
    ...
    groups = {
        str(key): value["road_segment_id"].astype(str).to_numpy()
        for key, value in static.groupby("road_highway", dropna=False)
    }
    positive_keys = set(
        zip(positives["road_segment_id"].astype(str), positives["datetime_hour"])
    )
    ...
    for item in positives[["road_segment_id", "datetime_hour"]].itertuples(index=False):
        ...
        candidates_primary = groups.get(
            str(highway_by_segment.get(road_segment_id, "unknown")), all_segments
        )
        for _ in range(ratio):
            picked: str | None = None
            for candidates, uses_fallback in (
                (candidates_primary, False),
                (all_segments, True),
            ):
            ...
            if picked is None:
                raise RuntimeError(
                    f"Cannot sample a negative at {timestamp} without positive-window overlap."
                )
            negative_keys.add((picked, timestamp))
            rows.append((picked, timestamp, np.int8(0)))
    negatives = pd.DataFrame(
        rows, columns=["road_segment_id", "datetime_hour", "target_24h"]
    )
...
def add_standard_history(data: pd.DataFrame, counts: pd.DataFrame) -> pd.DataFrame:
    result = data.copy()
    defaults = {
        "segment_accidents_total_prior": 0,
        "segment_accidents_prev_24h": 0,
        "segment_accidents_prev_7d": 0,
        "segment_accidents_prev_30d": 0,
        "segment_hours_since_prev_accident": -1.0,
        "segment_has_history": False,
    }
    ...
    for segment, indices in result.groupby("road_segment_id").groups.items():
        ...
        query_ns = (
            result.loc[row_indices, "datetime_hour"]
        ...
        for label, hours in (
            ...
            result.loc[row_indices, label] = total - prior(
        ...
        hours_since[has_history] = (
            query_ns[has_history] - event_ns[location[has_history] - 1]
        ) / 3_600_000_000_000
    ...
    for label, hours in (
        ...
        result[label] = total - city_prior(
            query_ns - np.int64(hours * 3_600_000_000_000)
        )
    return result


def cumulative_context_feature(
    data: pd.DataFrame,
    events: pd.DataFrame,
    ...
    right = events[
        ["road_segment_id", "event_hour", "event_count", *event_context]
    ].copy()
    right = right.groupby(
        ["road_segment_id", "event_hour", *event_context], as_index=False
    )["event_count"].sum()
    ...
    right["_cumulative"] = right.groupby(group_right, dropna=False)[
        "event_count"
    ].cumsum()
    left_sorted = left.sort_values(["datetime_hour", *group_right])
    merged = pd.merge_asof(
        left_sorted,
        right[["event_hour", *group_right, "_cumulative"]],
    ...
    values = pd.Series(
        merged["_cumulative"].fillna(0).to_numpy(dtype=np.int64),
        index=merged["_row_id"].to_numpy(),
        name=output,
    )
    return values.reindex(np.arange(len(data)), fill_value=0)


def add_seasonal_history(
    data: pd.DataFrame, counts: pd.DataFrame, calendar: pd.DataFrame
) -> pd.DataFrame:
    result = data.copy()
    calendar_context = calendar[["datetime_hour", "is_holiday"]].copy()
    calendar_context["is_new_year_period"] = (
        (calendar_context["datetime_hour"].dt.month == 12)
        & (calendar_context["datetime_hour"].dt.day >= 25)
    ) | (
        (calendar_context["datetime_hour"].dt.month == 1)
        & (calendar_context["datetime_hour"].dt.day <= 7)
    )
    ...
    event_context["event_hour_of_day"] = event_context["event_hour"].dt.hour.astype(
        "int8"
    )
    event_context["event_weekday"] = event_context["event_hour"].dt.weekday.astype(
        "int8"
    )
    ...
    result["segment_accidents_same_month_prior"] = cumulative_context_feature(
        result,
        event_context,
        ["_month"],
        ["event_month"],
    ...
    result["segment_accidents_same_hour_weekday_prior"] = cumulative_context_feature(
        result,
        event_context,
        ["_hour_of_day", "_weekday"],
        ["event_hour_of_day", "event_weekday"],
        "segment_accidents_same_hour_weekday_prior",
    ).to_numpy()
    result["segment_accidents_same_month_hour_weekday_prior"] = (
        cumulative_context_feature(
            result,
            event_context,
            ["_month", "_hour_of_day", "_weekday"],
    ...
    new_year_events = event_context.loc[
        event_context["is_new_year_period"].fillna(False)
    ].copy()
    ...
    result["segment_accidents_holiday_prior"] = cumulative_context_feature(
        result,
        holiday_events,
        ["_all_context"],
        ["_all_context"],
    ...
    result["segment_accidents_new_year_period_prior"] = cumulative_context_feature(
        result,
        new_year_events,
        ["_all_context"],
        ["_all_context"],
...
def build(args: argparse.Namespace) -> tuple[pd.DataFrame, dict[str, object]]:
    ...
    positives, outside_coverage = make_positive_windows(
        counts, feature_start, feature_end
    )
    negatives, sampling = sample_negatives(
    ...
    calendar["is_new_year_period"] = (
        (calendar["datetime_hour"].dt.month == 12)
        & (calendar["datetime_hour"].dt.day >= 25)
    ) | (
        (calendar["datetime_hour"].dt.month == 1)
        & (calendar["datetime_hour"].dt.day <= 7)
    )
    calendar_for_features = prefix_columns(calendar, "calendar_", {"datetime_hour"})
    data = data.merge(
        calendar_for_features,
        on="datetime_hour",
        how="left",
        validate="many_to_one",
    ...
    summary: dict[str, object] = {
        ...
        "sampling": sampling,
        "joins": {
            "road_unmatched": road_unmatched,
            "poi_unmatched": poi_unmatched,
            "calendar_unmatched": calendar_unmatched,
            "weather_unmatched": weather_unmatched,
        },
...
def write_report(data: pd.DataFrame, summary: dict[str, object], output: Path) -> Path:
    ...
    payload = summary | {
        ...
        "target_balance": {
            str(key): int(value)
            for key, value in data["target_24h"].value_counts().sort_index().items()
        },
        "temporal_coverage": {
        ...
        "leakage_policy": {
            ...
            "target_window_events_excluded_from_history": True,
        },
    }
...
def main() -> int:
    ...
    if (
        len(data.loc[data["target_24h"] == 0])
        != len(data.loc[data["target_24h"] == 1]) * args.negative_ratio
    ):
