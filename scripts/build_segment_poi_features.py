...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Create static POI features for ML road segments."
    )
...
def parse_linestring_midpoint(value: str) -> tuple[float, float] | None:
    ...
    try:
        ...
        points = [
            tuple(map(float, part.strip().split()[:2])) for part in body.split(",")
        ]
    ...
    for index, length in enumerate(lengths):
        if covered + length >= half and length:
            ...
            lon = points[index][0] + fraction * (
                points[index + 1][0] - points[index][0]
            )
            lat = points[index][1] + fraction * (
    ...
    with roads_path.open("r", encoding="utf-8-sig", newline="") as source:
        for row in csv.DictReader(source):
            ...
            if centre is not None:
                rows.append(
                    {
                        "road_segment_id": identifier,
                        "segment_longitude": centre[0],
                        "segment_latitude": centre[1],
                    }
                )
...
def haversine_m(
    lon: float, lat: float, poi_lon: np.ndarray, poi_lat: np.ndarray
) -> np.ndarray:
    lat1 = np.radians(lat)
    lon1 = np.radians(lon)
...
def build_features(centres: pd.DataFrame, pois: pd.DataFrame) -> pd.DataFrame:
    ...
    for centre in centres.itertuples(index=False):
        distances = haversine_m(
            centre.segment_longitude, centre.segment_latitude, poi_lon, poi_lat
        )
        ...
        for radius in RADII_METERS:
            ...
            for category in categories:
                feature_row[f"poi_{category}_{radius}m"] = int(
                    (within & (poi_categories == category)).sum()
                )
...
def main() -> int:
    ...
    if missing_geometry:
        raise ValueError(
            f"Road geometry is missing for {len(missing_geometry)} ML segments."
        )
    ...
    (report_dir / "poi_features_report.json").write_text(
