...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Create filtered ML-ready accident-road base data.")
    parser.add_argument("--input", type=Path, default=PROCESSED_ROOT / "accidents_with_roads_ml_base.parquet")
    parser.add_argument(
        "--max-distance-m",
        type=float,
        default=300.0,
    ...
    parser.add_argument("--output-parquet", type=Path, default=PROCESSED_ROOT / "accidents_with_roads_ml_filtered.parquet")
    parser.add_argument("--output-csv", type=Path, default=PROCESSED_ROOT / "accidents_with_roads_ml_filtered.csv")
    parser.add_argument("--suspicious-parquet", type=Path, default=PROCESSED_ROOT / "accidents_with_roads_ml_suspicious.parquet")
    parser.add_argument("--suspicious-csv", type=Path, default=PROCESSED_ROOT / "accidents_with_roads_ml_suspicious.csv")
...
def filter_ml_base(args: argparse.Namespace) -> dict[str, Any]:
    ...
    required = {"objectid", "distance_to_road_m", "road_segment_id", "accident_datetime"}
    ...
    valid_mask = data["road_segment_id"].notna() & data["accident_datetime"].notna() & distance.le(args.max_distance_m)
    ...
    filtered_confidence_counts = filtered["match_confidence"].value_counts(dropna=False).to_dict()
    return {
        ...
        "confidence_counts_before_filter": {str(key): int(value) for key, value in confidence_counts.items()},
        "confidence_counts_after_filter": {str(key): int(value) for key, value in filtered_confidence_counts.items()},
...
def write_report(report_dir: Path, summary: dict[str, Any]) -> None:
    (report_dir / "ml_filtering_summary.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
    lines = [
