    ...
    def __init__(
        ...
        *,
        ...
        self.api_key = (
            ...
            else (os.getenv("TOMTOM_API_KEY") or _local_api_key())
        )
    ...
    def get_segment(self, road_segment_id: str) -> dict[str, Any]:
        ...
        if not self.configured:
            return self._unavailable(
                road_segment_id, "tomtom_api_key_not_configured", point
            )
        try:
            ...
            LOGGER.warning(
                "TomTom live traffic unavailable for %s: %s", road_segment_id, exc
            )
        ...
        return {
            ...
            "congestion_ratio": None
            if not free_flow
            else float(current) / float(free_flow),
            "confidence": None
            if flow.get("confidence") is None
            else float(flow["confidence"]),
            "road_closure": flow.get("roadClosure"),
            "tomtom_frc": flow.get("frc"),
            "flow_coordinates": flow.get("coordinates", {}).get("coordinate", []),
        }

    def collect(
        self, road_segment_ids: Iterable[str], output_path: Path
    ) -> list[dict[str, Any]]:
        ...
        rows = [
            {
                ...
                "coordinates": None
                if not item.get("query_coordinates")
                else (
        ...
        if output_path.exists():
            snapshot = pd.concat(
                [pd.read_parquet(output_path), snapshot], ignore_index=True
            )
