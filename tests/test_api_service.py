...
class ApiServiceTests(unittest.TestCase):
    def setUp(self) -> None:
        def predictor(_: str) -> pd.DataFrame:
            return pd.DataFrame([{
                "road_segment_id": "segment-1", "priority_rank": 1,
                "operational_priority": "high", "reasons": ["DYNAMIC_TOP_5PCT"],
            }])

        self.training = TrainingRuntime(lambda _: [{"id": "event-1"}])
        self.client = TestClient(create_app(
            api_key="shared-secret", runtime=Runtime(predictor), training=self.training
        ))
    ...
    def test_predict_and_read_back_top_risk(self):
        response = self.client.post("/api/v1/predict", headers=self.headers, json={"force": False})
        ...
        recommendations = self.client.get("/api/v1/recommendations/top?limit=1", headers=self.headers)
    ...
    def test_training_is_idempotent_and_queryable(self):
        ...
        status = self.client.get(f"/api/v1/training/{first.json()['trainingRunId']}", headers=self.headers)
        self.assertEqual(status.status_code, 200)
        self.assertEqual(status.json()["status"], "QUEUED")

    def test_training_rejects_parallel_run(self):
        body = {"baseDatasetSnapshotId": "snapshot", "includeConfirmedEventsUntil": "2026-07-14T12:00:00Z"}
        self.client.post("/api/v1/training", headers=self.headers | {"Idempotency-Key": "one"}, json=body)
        response = self.client.post("/api/v1/training", headers=self.headers | {"Idempotency-Key": "two"}, json=body)
